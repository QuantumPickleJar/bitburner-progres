...
export function calcBuffer(ns) {
    ...
    ns.tprint(`Allocated $${allocatedFunds} for stocks.`)
...
export async function main(ns) {
    // @todo: if argument is passed, check if it's for verbose mode
...
function printHeartbeat(ns) { 
    ...
    for (let stock of stockObjects) {
        if (stock.ownedLongShares > 0) {
            ns.tprint(`{${stock.sym}, ${stock.ownedLongShares}},`);
        }
    }
    ns.tprint("]");
...
function purchaseLongStocks(ns, stock, budget) {
    ...
    if (numStocksToBuy > 0) {
        ...
        ns.tprint(`Purchased ${numStocksToBuy} for $${transactionCost}`);
        return transactionCost;
    } else if (numStocksToBuy <= 0) {

        if (stocksAvailableForPurchase <= 0) {
            // @todo: verbose arg check gate needed
            ns.tprint(`${stock.sym} at max cap: skipping...`)
        } else if (budget <= pricePerStock) {
            // todo: guard with a verbose-only mode gate
            ns.tprint(`Insufficient funds to buy ${stock.sym}.
                       Price:                    ${pricePerStock}
                       Budget:                   ${budget}`);
